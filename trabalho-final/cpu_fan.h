...
private:GLfloat c_center[3] = { 0.0075, 0.0075, 0.0075};
		...
		GLfloat fan_spin_theta = 0, y_spin = 0;///variavel que controla a ventuinha
		...
		// Desenho das partes que compõem a ventoinha
		void draw_fan_blades(bool onOff);
		void draw_fan_center();
		void draw_fan_rim();
		void draw_cooler();
		void showDescrpiton();image.png
		void motionHandle();
public: void render(bool onOff);
};
...
void cpu_fan::render(bool onOff) {
	motionHandle();
	if (!visible) return;
	glPushMatrix();
...
void cpu_fan::draw_fan_blades(bool onOff) {
	glPushMatrix();
	...
	if(onOff){
		fan_spin_theta += deltaTime * 0.1; //AQUI PARA A VENTUINHA 
...
void cpu_fan::draw_fan_rim() {
	...
	float rimScale_out = 19.8, rimScale_in = 19.;
	glBegin(GL_QUAD_STRIP);
	for (int i = 0; i <= sides; i++) {
